...
import numpy as np
import torch
from albumentations import (Compose, HorizontalFlip, VerticalFlip, GaussNoise, MotionBlur, Normalize,
...
class PermuteChannels(BasicTransform):
    ...
    def __call__(self, force_apply=True, **data):
        ...
        # 不要對 mask 做 transpose
        data["image"], data["mask"] = image, mask
        # print(f"[{self.__class__.__name__}] input image shape:", image.shape)
        return data
