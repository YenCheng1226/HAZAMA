...
from ml4floods.preprocess.transformations import Compose
...
from albumentations import Resize, HorizontalFlip, RandomRotate90
...
def get_transformations(data_config) -> Tuple[Callable, Callable]:
    ...
    # === 設定 train_transform ===
    transform_list = [
        transformations.InversePermuteChannels(),  # (C,H,W) -> (H,W,C)
        Resize(256, 256),
        RandomRotate90(always_apply=True, p=0.5),
        HorizontalFlip(always_apply=True, p=0.5),
    ]

    if channel_mean is not None:
        transform_list.append(
            transformations.Normalize(mean=channel_mean, std=channel_std, max_pixel_value=1)
        )

    transform_list.extend([
        transformations.PermuteChannels(),  # (H,W,C) -> (C,H,W)
        transformations.ToTensor()
    ])

    train_transform = Compose(transform_list, is_check_shapes=False)
