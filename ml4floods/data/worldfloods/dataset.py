...
class WorldFloodsDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # print("This is mask shape(in dataset)",mask_tif.shape)
        ...
        # print(f"[DEBUG] index={idx}, image shape={image.shape}, mask shape={mask.shape}")
...
class WorldFloodsDatasetTiled(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # 不要轉換 image shape，保持 (C, H, W)
        # image = np.transpose(image, (1, 2, 0))  # <-- 請移除這行
        ...
        # print("Before transform, image shape:", image.shape)
        # print("Before transform, mask shape:", mask.shape)


        # Apply transformation
        if self.transforms is not None:
            data = self.transforms(image=image, mask=mask)
            data["filename"] = image_name  # 加這行
        else:
            data = {"image": image, "mask": mask, "filename": image_name}  # 加 filename
        return data
